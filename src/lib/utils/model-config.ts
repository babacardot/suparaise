...
export const BROWSER_USE_MODELS = {
  ...
  'gemini-2.5-flash': { cost: 0.01, performance: 'excellent', recommended: true }, // Best price/performance
  ...
  'claude-sonnet-4-20250514': { cost: 0.03, performance: 'excellent', recommended: false },

  // Future models (uncomment when available)
  'gpt-5-mini': { cost: 0.005, performance: 'excellent', recommended: true },
    'gemini-2.5-pro': { cost: 0.03, performance: 'excellent', recommended: false },
} as const
...
export const getModelForComplexity = (complexity: 'simple' | 'medium' | 'complex'): SupportedModel => {
  return MODEL_CONFIG.byComplexity[complexity]
}

// Get cost estimate for a model
export const getModelCost = (model: SupportedModel, steps: number = 50): number => {
