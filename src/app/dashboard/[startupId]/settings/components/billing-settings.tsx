...
export default function BillingSettings() {
    ...
    const handleSubscribe = async (plan: 'pro_monthly' | 'pro_yearly' | 'max_monthly' | 'max_yearly') => {
        setIsLoading(true)
        try {
                    ...
                    <div className="bg-zinc-50 dark:bg-zinc-900/30 border border-zinc-200 dark:border-zinc-800 rounded-sm p-6">
                        ...
                        {isSubscribed ? (
                            <p className="text-muted-foreground">
                                ...
                                {periodEndDate && ` and renews on ${periodEndDate.toLocaleDateString()}`}
                            </p>
                        ) : (
                            <div className="space-y-2">
                                <p className="text-muted-foreground">You&apos;re on the free plan.</p>
                                <p className="text-sm text-muted-foreground">
                                    Upgrade to Pro for 100 runs per month, access to 2000+ funds, and advanced features.
                                </p>
                            </div>
                        )}
                    </div>
