...
interface ApplicationsPageClientProps {
  ...
  initialFilters: Omit<ApplicationsFiltersType, 'dateRange'> & {
    dateFrom?: string
    dateTo?: string
  }
...
export default function ApplicationsPageClient({
  ...
  const handleFiltersChange = useCallback(
    (newFilters: ApplicationsFiltersType) => {
      ...
      if (newFilters.statusFilter.length > 0) {
        newFilters.statusFilter.forEach((status) =>
          params.append('statusFilter', status),
        )
      }
      if (newFilters.typeFilter.length > 0) {
        newFilters.typeFilter.forEach((type) =>
  ...
  const handleRetrySubmission = useCallback(
    async (submission: ApplicationSubmission) => {
      ...
      setRetryingSubmissions(
        (prev) => new Set(Array.from(prev).concat(submission.submission_id)),
      )
