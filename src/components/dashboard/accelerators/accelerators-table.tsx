...
const AcceleratorsTable = React.memo(function AcceleratorsTable({
  ...
  const bannerContent = React.useMemo(() => {
    ...
    if (level === 'FREE') {
      return {
        text: 'Upgrade to Pro to unlock over 1,000 more accelerators and advanced agent capabilities.',
      }
    }
    if (level === 'PRO') {
      return {
        text: 'Upgrade to Max to access our full database of 2,000+ accelerators and powerful developer tools.',
      }
    }
    return null
  }, [subscription?.permission_level])

  const filteredAccelerators = React.useMemo(() => accelerators, [accelerators])
    ...
    switch (docType) {
      ...
      case 'traction_data':
        return {
          ...
          customColor: [0.543, 0.361, 0.839] as [number, number, number],
        }
      case 'business_plan':
        return {
          label: 'Business Plan',
  ...
  const handleApplyForm = React.useCallback(
    ...
    [user?.id, startupId, submittingAccelerators, toast, queueStatus],
  )
                            ...
                            {accelerator.submission_type === 'other' && (
                              ...
                              >
                                <LottieIcon
                                  ...
                                  className="mr-1"
                                  isHovered={
                                    hoveredButton === `learn-${accelerator.id}`
                                  }
                                />
                                Learn
                              </Button>
