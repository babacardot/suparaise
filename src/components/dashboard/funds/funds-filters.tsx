...
export default function FundsFilters({
    ...
    const getStageColor = (stage: string) => {
        switch (stage) {
            case 'Pre-seed': return "bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300 hover:bg-rose-100 dark:hover:bg-rose-900/40"
            case 'Seed': return "bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/40"
            case 'Series A': return "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/40"
            case 'Series B': return "bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40"
            case 'Series C': return "bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/40"
            case 'Growth': return "bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-900/40"
            default: return "bg-slate-50 text-slate-700 dark:bg-slate-900/30 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900/40"
        }
    }

    const getIndustryColor = (industry: string) => {
        // Tech/SaaS group
        if (['B2B SaaS', 'AI/ML', 'Deep tech', 'Developer tools', 'Cybersecurity'].includes(industry)) {
        ...
        // Consumer/Commerce
        if (['Consumer', 'E-commerce', 'Marketplace', 'Gaming'].includes(industry)) {
    ...
    const getOptionColors = (filterKey: keyof FundsFilters, value: string) => {
        if (filterKey === 'submissionTypes') {
            if (value === 'form') return "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/40"
            if (value === 'email') return "bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/40"
            if (value === 'other') return "bg-pink-50 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300 hover:bg-pink-100 dark:hover:bg-pink-900/40"
        }
        if (filterKey === 'formComplexity') {
            if (value === 'simple') return "bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300 hover:bg-teal-100 dark:hover:bg-teal-900/40"
            if (value === 'standard') return "bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 hover:bg-yellow-100 dark:hover:bg-yellow-900/40"
            if (value === 'comprehensive') return "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/40"
        }
        ...
        if (filterKey === 'requiredDocuments') {
            if (value === 'pitch_deck') return "bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 hover:bg-yellow-100 dark:hover:bg-yellow-900/40"
            if (value === 'video') return "bg-red-50 text-red-700 dark:bg-red-900/30 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/40"
            if (value === 'financial_projections') return "bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300 hover:bg-teal-100 dark:hover:bg-teal-900/40"
            if (value === 'traction_data') return "bg-violet-50 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300 hover:bg-violet-100 dark:hover:bg-violet-900/40"
            if (value === 'team_bios') return "bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40"
        }
        return "bg-slate-50 text-slate-700 dark:bg-slate-900/30 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-900/40"
    }
            ...
            <div className="flex flex-wrap items-center gap-4">
                ...
                {columnVisibility.region && (
                    <div className="w-full sm:w-44">
                        <Popover>
                                    ...
                                    <div className="flex items-center space-x-2 truncate">
                                        {filters.regionFocus.length > 0 ? (
                                            filters.regionFocus.slice(0, 2).map((region) => {
                                                const regionColor = getRegionColor(region).split(' hover:')[0] // Remove hover classes for display
                                                return (
                                                    <Badge
                                                        key={region}
                            ...
                            <PopoverContent className="w-44 p-0 bg-card text-card-foreground rounded-sm" align="start">
                                <FilterSection
                ...
                {columnVisibility.focus && (
                    <div className="w-full sm:w-36">
                        <Popover>
                                    ...
                                    <div className="flex items-center space-x-2 truncate">
                                        {filters.stageFocus.length > 0 ? (
                                            filters.stageFocus.slice(0, 2).map((stage) => {
                                                const stageColor = getStageColor(stage).split(' hover:')[0] // Remove hover classes for display
                                                return (
                                                    <Badge
                                                        key={stage}
                            ...
                            <PopoverContent className="w-36 p-0 bg-card text-card-foreground rounded-sm" align="start">
                                <FilterSection
                ...
                {columnVisibility.industry && (
                    <div className="w-full sm:w-40">
                        <Popover>
                                    ...
                                    <div className="flex items-center space-x-2 truncate">
                                        {filters.industryFocus.length > 0 ? (
                                            filters.industryFocus.slice(0, 2).map((industry) => {
                                                const industryColor = getIndustryColor(industry).split(' hover:')[0] // Remove hover classes for display
                                                return (
                                                    <Badge
                                                        key={industry}
                            ...
                            <PopoverContent className="w-40 p-0 bg-card text-card-foreground rounded-sm" align="start">
                                <FilterSection
                                    ...
                                    <div className="flex items-center space-x-2 truncate">
                                        {filters.submissionTypes.length > 0 ? (
                                            filters.submissionTypes.map((type) => {
                                                const typeOption = SUBMISSION_TYPES.find(t => t.value === type)
                                                const color = type === 'form'
                    ...
                    <div className="w-full sm:w-40">
                        <Popover>
                            <PopoverTrigger asChild>
                                ...
                                >
                                    <div className="flex items-center space-x-2 truncate">
                                        {filters.requiredDocuments && filters.requiredDocuments.length > 0 ? (
                                            filters.requiredDocuments.slice(0, 2).map((doc) => {
                                                const docOption = REQUIRED_DOCUMENTS.find(d => d.value === doc)
                                                const docColor = getOptionColors('requiredDocuments', doc).split(' hover:')[0] // Remove hover classes for display
                                                return (
                                                    <Badge
                                                        key={doc}
                                        ...
                                        {filters.requiredDocuments && filters.requiredDocuments.length > 2 && (
                                            <Badge className="ml-1 bg-slate-50 text-slate-700 dark:bg-slate-900/30 dark:text-slate-300 rounded-sm">
                ...
                <div className="w-full sm:w-auto">
                    <Popover>
                        ...
                        <PopoverContent className="w-48 p-2 bg-card text-card-foreground rounded-sm" align="end">
                                ...
                                {Object.entries(columnVisibility).map(([key, visible]) => (
                                    <div
                                        ...
                                        onClick={(e) => {
                                            e.preventDefault()
                                            e.stopPropagation()
                                            onColumnVisibilityChange(key as keyof ColumnVisibility, !visible)
                                        }}
                                        className={`
                                            flex items-center px-3 py-2 rounded-sm cursor-pointer transition-colors text-left
                                            ${visible
